...
def get_active_users() -> User:
    ...
    ten = timedelta(10)
    active_users = User.objects.filter(last_login__gte=(today - ten))
    return active_users


def get_amount_users() -> User:
    """Retorne a quantidade total de usuarios do sistema """
    amount_user = User.objects.all().count()
    return amount_user


def get_admin_users() -> User:
    """Traga todos os usuarios com grupo = 'admin"""
    admin_users = User.objects.filter(group__name='admin')
    return admin_users
...
def get_all_events_by_group() -> Group:
    ...
    events_by_group = Group.objects.filter(user__agent__event__level='information')
